...
function handleFieldControls(e) {
    ...
    const isRowControl = button.classList.contains("append-row") || button.classList.contains("remove-row");
    const isColumnControl = button.classList.contains("append-column") || button.classList.contains("remove-column");
    if (isRowControl) handleRows(button);
    if (isColumnControl) handleColumns(button);
    updateButtonStates();
}
function handleRows(button) {
    ...
    if (button.classList.contains("remove-row") && fieldBody.rows.length > 2) fieldBody.lastElementChild.remove();
}
function handleColumns(button) {
    Array.from(fieldBody.rows).forEach((row)=>{
        ...
        if (button.classList.contains("remove-column") && row.cells.length > 2) row.lastElementChild.remove();
    });
}
